...
import { updateReleasesOrder } from "@/app/actions/ordering";
...
export default function ReorderReleasesPage() {
  ...
  function handleSave() {
    ...
    setSaveError(null);
    startSave(async () => {
      const result = await updateReleasesOrder(releases.map((r) => r.id));
      if (result.success) {
        setSaveStatus("saved");
      } else {
        setSaveStatus("error");
                ...
                >
                  <div style={{ display: "flex", flexDirection: "column", gap: "3px" }}>
                    ...
                    <button
                      ...
                      disabled={index === releases.length - 1 || isSaving}
                      style={{
                        ...mono,
                        fontSize: "9px",
                        background: "none",
                        border: "none",
                        color: index === releases.length - 1 ? "rgba(90,158,212,0.25)" : "#5A9ED4",
                        cursor: index === releases.length - 1 || isSaving ? "default" : "pointer",
                        padding: "0 4px",
                        lineHeight: 1,
                      }}
                    >
