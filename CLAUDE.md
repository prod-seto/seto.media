...
- TypeScript 5 / Next.js 16.2.1 App Router + React 19 + `@supabase/supabase-js` 2.x, `@supabase/ssr`, `next/font/google` (all existing) (006-ps2-disc-redesign)
- Supabase PostgreSQL — `tools` table (12 new columns via ALTER TABLE) (006-ps2-disc-redesign)
- Supabase PostgreSQL — `tools` table (no schema changes) (008-tools-page-redesign)
- TypeScript 5 / Next.js 16.2.1 App Router + React 19 + Web Audio API (browser native — `AudioContext`, `decodeAudioData`, `OfflineAudioContext`); no new npm packages (009-bpm-key-detector)
- N/A — feature is fully client-side and stateless (009-bpm-key-detector)
...
## Recent Changes
- 009-bpm-key-detector: Added TypeScript 5 / Next.js 16.2.1 App Router + React 19 + Web Audio API (browser native — `AudioContext`, `decodeAudioData`, `OfflineAudioContext`); no new npm packages
- 008-tools-page-redesign: Added TypeScript 5 / Next.js 16.2.1 App Router + React 19 + `@supabase/supabase-js` 2.x, `@supabase/ssr`, `next/font/google` (all existing)
- 006-ps2-disc-redesign: Added TypeScript 5 / Next.js 16.2.1 App Router + React 19 + `@supabase/supabase-js` 2.x, `@supabase/ssr`, `next/font/google` (all existing)
